...
const YjsProjectManagerMixin = {
  ...
  applyMixin(projectManager) {
    ...
    projectManager.enableYjsMode = async function (projectId, authToken, options = {}) {
      ...
      // Initialize bridge - but DON'T enable autoSync yet
      // We need to capture baseline FIRST, then enable autoSync
      // Note: autoSync: false MUST come after ...options to ensure it's never overridden
      this._yjsBridge = await window.YjsModules.initializeProject(projectId, authToken, {
        ...
        ...options,
        autoSync: false, // Defer autoSync until after baseline capture (MUST be last)
      });
      ...
      // Capture baseline state NOW - after ALL initialization is complete
      // This includes everything in YjsProjectBridge.initialize() PLUS:
      // - odeTitleElement.initYjsBinding() (may import values with 'initial' origin)
      // - themes.initYjsBinding() (may import values with 'initial' origin)
      // This ensures accurate change detection so users don't see "unsaved changes"
      ...
      if (this._yjsBridge?.documentManager?.captureBaselineState) {
        ...
        Logger.log('[ProjectManager] Baseline state captured after all bindings');
      }

      // NOW enable autoSync - AFTER baseline is captured
      // This ensures hasUnsavedChanges() returns false for fresh projects
      if (this._yjsBridge?.enableAutoSync) {
        this._yjsBridge.enableAutoSync();
        Logger.log('[ProjectManager] AutoSync enabled after baseline capture');
      }
