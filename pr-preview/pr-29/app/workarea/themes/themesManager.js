        ...
        if (project?._yjsBridge) {
            ...
            if (documentManager) {
                ...
                if (initialTheme) {
                    ...
                    // Use save=false to not modify Yjs during initialization
                    // This prevents marking the document as dirty before baseline is captured
                    // The theme will be saved when the user makes actual changes
                    const defaultTheme = window.eXeLearning?.config?.defaultTheme || 'base';
                    this.selectTheme(defaultTheme, false, false, false);
                    getLogger().log('[ThemesManager] No theme in project, applying default without save:', defaultTheme);
                }
