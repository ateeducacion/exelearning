...
class YjsDocumentManager {
  ...
  constructor(projectId, config = {}) {
    ...
    // Baseline state for change detection (state vector comparison)
    // Used to detect actual changes vs initialization changes
  ...
  async initialize(options = {}) {
    ...
    // NOTE: captureBaselineState() is NOT called here intentionally.
    // It is called at the END of YjsProjectBridge.initialize() AFTER:
    // - ensureBlankStructureIfEmpty() adds blank pages
    // - waitForWebSocketSync() completes
    // - All remote updates are applied
    // This ensures the baseline captures the fully-initialized document state,
    // not a partial/empty state that would cause false "unsaved changes" detection.
    ...
    this.ydoc.on('update', (update, origin) => {
      // Don't mark dirty for system/initialization updates
      // 'system' - explicit system updates during initialization
      // 'initial' - y-indexeddb/y-websocket sync updates during initialization
      // Mark dirty for both local changes AND remote changes from collaborators
      const isInitOrigin = origin === 'system' || origin === 'initial';
      if (!isInitOrigin) {
        this.markDirty();
      }
    });
  ...
  async waitForWebSocketSync() {
    ...
    if (!otherUsersPresent) {
      ...
      // Even for single user, wait for initial sync to complete
      // This prevents race conditions where sync completes after baseline capture
  ...
  /**
   * Wait for initial WebSocket sync (brief wait for connection to stabilize)
   * Used in single-user scenarios to ensure any pending sync completes
   * before baseline capture
   * @private
   */
  async _waitForInitialSync() {
    // If already synced, return immediately
    if (this.wsProvider?.synced) {
      Logger.log('[YjsDocumentManager] WebSocket already synced (initial)');
      return;
    }

    // Wait briefly for sync to complete (500ms max)
    const timeout = 500;
    return new Promise((resolve) => {
      const timer = setTimeout(() => {
        Logger.log('[YjsDocumentManager] Initial sync timeout, proceeding');
        resolve();
      }, timeout);

      // wsProvider may not have 'once' method in test mocks
      if (this.wsProvider?.once) {
        this.wsProvider.once('sync', (isSynced) => {
          if (isSynced) {
            clearTimeout(timer);
            Logger.log('[YjsDocumentManager] Initial sync complete');
            resolve();
          }
        });
      } else {
        // No once method - just use the timeout
        // (tests or providers without event support)
      }
    });
  }
  ...
  /**
   * Capture the current document state as the baseline for change detection.
   * Call this after initialization completes to establish the "clean" state.
   * Uses Yjs state vector for efficient comparison (~10-50 bytes).
   *
   * Also enables dirty tracking (by clearing _suppressDirtyTracking) and resets
   * isDirty to false. This establishes a clean state where future changes will
   * be properly tracked.
   */
  captureBaselineState() {
    if (!this.ydoc || !this.Y) {
      console.warn('[YjsDocumentManager] Cannot capture baseline: ydoc not initialized');
      return;
    }

    this.baselineStateVector = this.Y.encodeStateVector(this.ydoc);
    this.baselineStateVectorString = Array.from(this.baselineStateVector).join(',');

    // Enable dirty tracking now that initialization is complete
    // Any changes from this point forward will be properly tracked as dirty
    this._suppressDirtyTracking = false;

    // Reset isDirty since the current state is now the baseline (clean state)
    // Any dirty state from before would be stale (shouldn't happen with suppression)
    if (this.isDirty) {
      this.isDirty = false;
      Logger.log('[YjsDocumentManager] Dirty state reset after baseline capture');
    }
  ...
  /**
   * Check if the document has actual changes compared to the baseline state.
   * Uses state vector comparison for accurate detection.
   * @returns {boolean} true if document differs from baseline
   */
  hasActualChanges() {
    if (!this.ydoc || !this.Y || !this.baselineStateVector) {
      // Fallback to isDirty if baseline not available
      return this.isDirty;
    }
    ...
    return currentStateVectorString !== this.baselineStateVectorString;
  }
   ...
   */
  markDirty() {
    // Don't mark dirty during initialization (before baseline is captured)
    // This prevents initialization changes (server load, WebSocket sync) from showing the red dot
   ...
   * the unsaved indicator, while still supporting undo-to-baseline detection.
   * @returns {boolean}
   */
  hasUnsavedChanges() {
    ...
    // isDirty is true - user made changes at some point
    // Use state vector comparison to detect if undo restored to baseline
    if (this.baselineStateVector) {
      return this.hasActualChanges();
    }

    // Fallback to isDirty flag if no baseline
    return true;
  }
