...
**Current State**: Active migration from Symfony (v3.0 production) to NestJS (v4.0 development on `exe4` branch). Both backends coexist with Electron desktop packaging.

## Architecture

### Dual Backend System

The Electron main process (`electron/main.js`) detects the `USE_NEST_BACKEND` environment flag and spawns either:
- **Symfony backend** (legacy, port 8080) - PHP/Twig-based monolith
- **NestJS backend** (new, port 3000) - TypeScript modular architecture
...
### NestJS Development (nest-backend/)
...
### Symfony Development (root/)

```bash
# Docker containers
make up                # Start (foreground)
make upd               # Start (background, wait for health)
make down              # Stop
...
composer phpunit-unit  # Unit tests
composer phpunit-e2e   # E2E with Panther
```
...
`/src/Service/net/exelearning/Service/Api/OdeService.php` (3,280 lines) is being split into:
...
See `nest-backend/MIGRATION_PLAN.md` for detailed roadmap (24-34 hour estimate).
...
1. `nest-backend/.env` (local overrides)
...
- `NEST_PORT` - Server port (default: 3000)
...
# Check electron/main.js for IPC events
```
...
### Documentation
- `nest-backend/MIGRATION_PLAN.md` - Complete migration roadmap
- `/ARCHITECTURE_PROMPT.md` - Full architecture specification (967 lines)
- `nest-backend/CLAUDE.md` - This file
...
### Electron
- `electron/main.js` - Main process (52KB, 1,413 lines)
- `electron/preload.js` - Preload script
- `electron/update-manager.js` - Auto-update logic
...
### Configuration
- `nest-backend/.env` - Local environment overrides
- `nest-backend/tsconfig.json` - TypeScript config
- `nest-backend/jest.config.js` - Jest config
- `/Makefile` - Docker/Symfony commands (33KB, 1,000+ lines)
...
- `USE_NEST_BACKEND=1` required for Electron to use NestJS
