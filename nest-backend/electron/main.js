...
function initializeEnv() {
  ...
  const desiredPort = process.env.NEST_PORT || process.env.APP_PORT || '3000';
...
function waitForServer(callback) {
  ...
  const checkServer = () => {
    ...
    req.on('error', () => {
      console.log('Application server not available, retrying...');
      setTimeout(checkServer, 1000);  // Try again in 1 second
    });
