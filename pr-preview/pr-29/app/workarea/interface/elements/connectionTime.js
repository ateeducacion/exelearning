    ...
    setLastUpdatedToElement() {
        if (this.lastUpdatedDate) {
            ...
            if (
                ...
                // Only add 'unsaved' class if NOT using Yjs save detection
                // When Yjs is active, save detection is handled by YjsProjectBridge.updateSaveStatus()
                // which uses state vector comparison for accurate change detection
                const hasYjsBridge = !!eXeLearning?.app?.project?._yjsBridge;
                if (!hasYjsBridge) {
                    $('#head-top-save-button')
