...
# Run the linter to check TS and JS code style
lint: lint-ts lint-js

# Automatically fix TS and JS code style issues
fix: fix-ts fix-js


# Check TypeScript files indentation
lint-ts:
	echo "TO-DO"
...
# Indent TypeScript files with 4 spaces
fix-ts:
	echo "TO-DO"
...
# Run the app locally in development mode with Electron
# Automatically starts NestJS backend and launches Electron
run-app: fail-on-windows
	@echo "🚀 Launching eXeLearning with NestJS backend..."
	@set -e; \
	CLEANED=0; \
	cleanup() { \
		[ "$$CLEANED" = "1" ] && return; \
	...
	sleep 5; \
	if ! kill -0 $$NEST_PID 2>/dev/null; then \
		echo "❌ NestJS failed to start. Check the output above for errors."; \
		exit 1; \
	fi; \
	echo "✅ NestJS ready!"; \
...
ifndef VERSION
	...
	@echo "📦 Packaging eXeLearning v$(PACKAGE_VERSION)..."
...
help:
	...
	@echo "  up-local               - Start NestJS migration environment (Symfony + NestJS + Proxy)"
	...
	@echo "  stop-local             - Stop NestJS migration environment"
	@echo ""
	...
	@echo "Packaging:"
	@echo ""
	@echo "  run-app               - Run the app locally in Electron development mode"
	@echo "  package               - Package the app with electron-builder (usage: make package VERSION=x.y.z [PUBLISH=always])"
	@echo ""
