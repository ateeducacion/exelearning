...
class YjsProjectBridge {
  ...
  async initialize(projectId, authToken, options = {}) {
    ...
    // NOTE: captureBaselineState() is NOT called here.
    // It is called at the END of YjsProjectManagerMixin.enableYjsMode() AFTER:
    // - odeTitleElement.initYjsBinding()
    // - themes.initYjsBinding()
    // - All property bindings that might import values with 'initial' origin
    // This ensures the baseline captures the FINAL initialized state.
  ...
  injectSaveStatusUI() {
    ...
    // NOTE: Initial save status is NOT set here because:
    // 1. At this point, captureBaselineState() hasn't been called yet
    // 2. hasUnsavedChanges() would fall back to isDirty which may be true from init
    // 3. The proper place to set initial status is in enableAutoSync() which is called
    //    AFTER initialize() completes (and thus after baseline is captured)
  }
  ...
  updateSaveStatus(status, message = null) {
    ...
    if (this.saveButton) {
      ...
      switch (status) {
        case 'saved':
          // Normal state - no class added (button returns to default appearance)
          // The classList.remove above already cleared all classes
          break;
        case 'saving':
...
<rect x="0.5" y="0.5" width="39" height="39" rx="5.5" stroke="#9ca3af" stroke-dasharray="5 5"/>
  ...
  enableAutoSync() {
    ...
    // Set initial status based on actual document changes
    // Use hasUnsavedChanges() which uses state vector comparison
    if (this.documentManager.hasUnsavedChanges()) {
      this.updateSaveStatus('unsaved');
    } else {
      // Explicitly set to 'saved' to clear any 'unsaved' state from initialization
      this.updateSaveStatus('saved');
    }
