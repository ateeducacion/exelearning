...
export class XmlBuilderService {
   ...
   */
  private buildNavigationFromPages(
    pages: NormalizedPage[],
  ): OdeXmlNavigation {
    // Find root pages (level 0 or no parent)
    const rootPages = pages.filter(
      (page) => page.level === 0 || page.parent_id === null,
    );
    ...
    // Build page tree
    const pageTree = rootPages.map((rootPage) =>
      this.buildPageTree(rootPage, pages),
    );
  ...
  private buildPageTree(
    ...
    if (page.components && page.components.length > 0) {
      const components = page.components.map((comp) => ({
        '@_type': comp.type,
        '@_id': comp.id,
        '@_position': comp.position,
        content: comp.content || undefined,
        properties: Object.keys(comp.properties || {}).length > 0
          ? comp.properties
          : undefined,
    ...
    if (children.length > 0) {
      const childPages = children.map((child) =>
        this.buildPageTree(child, allPages),
      );
